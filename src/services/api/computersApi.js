...
export const searchComputers = ({
  ...
  apiClient.get("/computers/search", {
    params: { name, office, division, available, page, size, sort },
  });

// Wrapper used by AssetPage. Accepts the unified shape `{page, size, search, office, division}`
// and routes to /search whenever filters are present, /page otherwise.
export const listComputersPaged = ({
  page = 0,
  size = 12,
  search,
  office,
  ...
  const hasFilters = Boolean(search || office || division || available);
  if (hasFilters) {
    return searchComputers({ name: search, office, division, available, page, size, sort });
  }
  return listComputersPage({ page, size, sort });
};
